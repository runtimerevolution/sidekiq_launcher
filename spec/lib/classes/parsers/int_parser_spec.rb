...
RSpec.describe SidekiqLauncher::IntParser do
  let(:legal_strings) { %w[1 0 -1 99999999] }
  let(:illegal_strings) do
    ['', 'some_string', '1.0', '-1.0', 'true', '["some", "array"]', '[1, 0]', '{ some: 1, hash: 2 }',
     '{ "json": 1, "hash": 2 }']
  end
  ...
  it 'parses legal strings' do
    legal_strings.each do |str|
      expect(subject.try_parse(str)).to(be_a(Integer))
    end
  end
